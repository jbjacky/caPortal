...
@Component({
  selector: 'app-serchkeybase',
  templateUrl: './serchkeybase.component.html',
  styleUrls: ['./serchkeybase.component.css']
})
export class SerchkeybaseComponent implements OnInit, OnDestroy {
  ngOnDestroy(): void {
    ...
    if (searchVal) {
      if (searchVal.length >= 2) {
        if (this.boolCanSearch) {
          ...
          this.httpPostService.getWebApiData_GetDeptaByEmp(GetDeptaByEmp).subscribe((x: any) => {
            if (x.length > 0) {
              this.pushBase(x);
              if (this.base.length != 0) {
                ...
                this.httpPostService.getWebApiData_GetDeptaByEmp(GetDeptaByEmpC).subscribe((y: any) => {
                  if (y.length > 0) {
                    this.pushBase(y);
                    if (this.base.length == 0) {
                      // alert('此條件查不到員工')
