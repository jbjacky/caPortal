...
export class ChangeformComponent implements OnInit, AfterViewInit, OnDestroy {
                ...
                .subscribe(
                  (changeEmpDatax: any) => {
                    ...
                    for (let i = 0; i < this.uiShow.length; i++) {
                      if (this.uiShow[i].realDate == doFormatDate(this.searchDate)) {
  ...
  CheckAndNext() {
    ...
    for (let select of this.selectDay) {
      FlowShiftRoteCheck.ListShiftDate.push(select.realDate)
    }
    this.LoadingPage.show()
              ...
              .subscribe(
                (GetRoteByShiftData: GetRoteByShiftDataClass[]) => {
                  ...
                  var ShiftRoteCheckByTwoPerson: ShiftRoteCheckByTwoPersonClass = {
                    "EmpID1": this.Emp.EmpCode,
                    "EmpID2": this.ChangeEmp.EmpCode,
                    "DateB": this.uiShow[0].realDate,
                    "DateE": this.uiShow[this.uiShow.length - 1].realDate,
                    "ListShiftDate": [],
                    "IsDifferShift": true //true:等工時，false:不等工時
                  }
                  for (let select of this.selectDay) {
                    ShiftRoteCheckByTwoPerson.ListShiftDate.push(select.realDate)
                  }
