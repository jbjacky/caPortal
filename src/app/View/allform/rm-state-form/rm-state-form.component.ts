...
@Component({
  selector: 'app-rm-state-form',
  templateUrl: './rm-state-form.component.html',
  styleUrls: ['./rm-state-form.component.css']
})
export class RmStateFormComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  searchAttendExceptional(GetAttendExceptional) {
    this.LoadingPage.show()
    this.GetApiDataServiceService.getWebApiData_GetAttendExceptionalNew(GetAttendExceptional)
      .pipe(takeWhile(() => this.api_subscribe))
      .subscribe((data: any) => {
        // console.log(data)
        if (data) {
