...
@Component({
  selector: 'app-reviewform-detail-attend-unusualform',
  templateUrl: './reviewform-detail-attend-unusualform.component.html',
  styleUrls: ['./reviewform-detail-attend-unusualform.component.css']
})
export class ReviewformDetailAttendUnusualformComponent implements OnInit, OnDestroy {
  ngOnDestroy(): void {
  ...
  ngOnInit() {
    ...
    // console.log(this.ReviewformServiceService.AttendUnusualDetail)
    if (!this.ReviewformServiceService.AttendUnusualDetail) {
  ...
  checkforgetCardText_Approved() {
    ...
    // this.LoadingPage.show()
    // this.GetApiDataServiceService.getWebApiData_CardCheckByProcessFlowID(this.ReviewformServiceService.AttendUnusualDetail.ProcessFlowID)
    //   .pipe(takeWhile(() => this.api_subscribe))
    //   .subscribe(
    //     (x: string) => {
      ...
      .subscribe(
        y => {
          ...
          var FlowNodeFinish: FlowNodeFinishClass = {
            ProcessFlowID: parseInt(this.ReviewformServiceService.AttendUnusualDetail.ProcessFlowID),
            ProcessApParmAuto: parseInt(this.ReviewformServiceService.AttendUnusualDetail.ProcessApParmAuto),
            State: "3",
            FlowTreeID: this.ReviewformServiceService.AttendUnusualDetail.FlowTreeID,
            FlowNodeID: this.ReviewformServiceService.AttendUnusualDetail.FlowNodeID,
            Note: this.signText,
            NodeName: "核准",
            ManInfo: y[0],
            FlowDynamic: {
              FlowNode: this.ReviewformServiceService.AttendUnusualDetail.FlowNodeID,
  ...
  color_ActualOnTime() {
    if (this.GetAttendUnusualFlowAppsByProcessFlowIDData.RoteDateTimeB) {
      return '#028fcf'
    } else {
      return '#4c4c4c'
    }
  ...
  color_ActualOffTime() {
    if (this.GetAttendUnusualFlowAppsByProcessFlowIDData.RoteDateTimeE) {
      return '#028fcf'
    } else {
      return '#4c4c4c'
    }
