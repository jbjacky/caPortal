...
import { mergeMap, takeWhile, map, toArray } from 'rxjs/operators';
...
import { formatDateTime, doFormatDate_getMonthAndDay, doFormatDate } from 'src/app/UseVoid/void_doFormatDate';
import { GetShiftRoteTwoPersonAfterGetApiClass } from 'src/app/Models/PostData_API_Class/GetShiftRoteTwoPersonAfterGetApiClass';
import { ShiftRoteCountGetApiClass } from 'src/app/Models/PostData_API_Class/ShiftRoteCountGetApiClass';
import { from, BehaviorSubject, Observable } from 'rxjs';
import { CountReturnClass } from 'src/app/Models/CountReturnClass';
import { ShiftRoteCheckByWeekTypeGetApiClass } from 'src/app/Models/PostData_API_Class/ShiftRoteCheckByWeekTypeGetApiClass';
import { NoSameTimeShiftRoteSaveAndFlowStartClass } from 'src/app/Models/PostData_API_Class/NoSameTimeShiftRoteSaveAndFlowStartClass';
...
@Component({
  selector: 'app-change-two-pt',
  templateUrl: './change-two-pt.component.html',
  styleUrls: ['./change-two-pt.component.css']
})
export class ChangeTwoPTComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  uiShow_Simulation: uiShowClass[] = [] //模擬調班後
  ...
  nextPage() {
    if (this.selectDay.length == 0) {
      ...
      //取得班別代碼(依是否允許調班) -> 計算調班次數 -> 調班檢查(雙人) -> 判斷變形週期需要的例假及休息日 -> 模擬調班後 ->到最後確認頁面
      this.LoadingPage.show();
    ...
    this.GetApiDataServiceService.getWebApiData_GetRoteByShift('false')
      ...
      .subscribe(
        (GetRoteByShiftData: GetRoteByShiftDataClass[]) => {
          var canNext_GetRoteByShiftData = true
          for (let oneRote of GetRoteByShiftData) {
            for (let oneSelectDay of this.selectDay) {
              if (oneRote.RoteID.toString() == oneSelectDay.onePRoteID) {
                ...
                canNext_GetRoteByShiftData = false
          ...
          if (canNext_GetRoteByShiftData) {

            this.GetApiDataServiceService.getWebApiData_GetRoteDifferShift('false')
              .pipe(takeWhile(() => this.api_subscribe))
              .subscribe(
                (GetRoteDifferShift: GetRoteByShiftDataClass[]) => {

                  var canNext_RoteDiffer = true
                  for (let oneRote of GetRoteDifferShift) {
                    for (let oneSelectDay of this.selectDay) {
                      if (oneRote.RoteID.toString() == oneSelectDay.onePRoteID) {
                        alert(oneSelectDay.onePRoteName + '班型不允許不等工時調班')
                        canNext_RoteDiffer = false
                      } else if (oneRote.RoteID.toString() == oneSelectDay.twoPRoteID) {
                        alert(oneSelectDay.twoPRoteName + '班型不允許不等工時調班')
                        canNext_RoteDiffer = false
                      }
                    }
                  }
                  if (canNext_RoteDiffer) {
                    ...
                    var ShiftRoteCheckByTwoPerson: ShiftRoteCheckByTwoPersonClass = {
                      ...
                      "ListShiftDate": [],
                      "IsDifferShift": false //true:等工時，false:不等工時
                    }

                    var EmpShiftRoteCountArray: ShiftRoteCountGetApiClass[] = []
                    var ChangeEmpShiftRoteCountArray: ShiftRoteCountGetApiClass[] = []
                    ...
                    for (let select of this.selectDay) {
                      ...
                      EmpShiftRoteCountArray.push({
                        "EmpID": this.Emp.EmpCode,
                        "ShiftRoteDate": select.reallydate
                      })
                      ChangeEmpShiftRoteCountArray.push({
                        "EmpID": this.ChangeEmp.EmpCode,
                        "ShiftRoteDate": select.reallydate
                      })
                      ShiftRoteCheckByWeekTypeGetApi.ShiftRoteBySave.ShiftRoteDate.push({
                        "ShiftDate": select.reallydate,
                        "RoteID1": parseInt(select.onePRoteID),
                        "RoteID2": parseInt(select.twoPRoteID)
                      })
                    }
                    this.searchCount(EmpShiftRoteCountArray)//計算調班次數
                      .pipe(takeWhile(() => this.api_subscribe))
                      .subscribe(
                        (EmpCountDataArray: CountReturnClass[]) => {

                          var canNext_EmpCountDataArray: boolean = true
                          for (let EmpCountData of EmpCountDataArray) {
                            if (EmpCountData.count > 0) {
                              alert(EmpCountData.EmpID + ' ' + EmpCountData.ShiftDate + ' ' + '已有調班資料，不能再進行調班')
                              canNext_EmpCountDataArray = false
                            }
                          }
                          if (canNext_EmpCountDataArray) {
                            this.searchCount(ChangeEmpShiftRoteCountArray) //計算調班次數
                              .subscribe(
                                (ChangeEmpCountDataArray: CountReturnClass[]) => {

                                  var canNext: boolean = true
                                  for (let ChangeEmpCountData of ChangeEmpCountDataArray) {
                                    if (ChangeEmpCountData.count > 0) {
                                      alert(ChangeEmpCountData.EmpID + ' ' + ChangeEmpCountData.ShiftDate + ' ' + '已有調班資料，不能再進行調班')
                                      canNext = false
                                    }
                                  }
                                  if (canNext) {
                                    //調班檢查(雙人) -> 判斷變形週期需要的例假及休息日 -> 模擬調班後 ->到最後確認頁面
                                    this.GetApiDataServiceService.getWebApiData_ShiftRoteCheckByTwoPerson(ShiftRoteCheckByTwoPerson)
                                      .pipe(takeWhile(() => this.api_subscribe))
                                      .subscribe(
                                        (x: string) => {
                                          if (x.length == 0) {
                                            this.ShiftRoteCheckByWeek(ShiftRoteCheckByWeekTypeGetApi);
                                          } else {
                                            alert(x)
                                            this.LoadingPage.hide()
                                          }
  ...
  searchCount(ShiftRoteCountArray) {
    // 計算調班次數
    return from(ShiftRoteCountArray)
      .pipe(takeWhile(() => this.api_subscribe))
      .pipe(
        mergeMap((ShiftRoteCountGetApi: ShiftRoteCountGetApiClass) =>
          this.GetApiDataServiceService.getWebApiData_ShiftRoteCount(ShiftRoteCountGetApi)
            .pipe(takeWhile(() => this.api_subscribe))
            .pipe(
              map((r: any) => {
                var CountReturnData: CountReturnClass = {
                  EmpID: ShiftRoteCountGetApi.EmpID,
                  ShiftDate: ShiftRoteCountGetApi.ShiftRoteDate,
                  count: r
                }
                return CountReturnData
              })
            )
        ), toArray()
      )
  }
  private ShiftRoteCheckByWeek(ShiftRoteCheckByWeekTypeGetApi: ShiftRoteCheckByWeekTypeGetApiClass) {
    //判斷變形週期需要的例假及休息日 -> 模擬調班後 ->到最後確認頁面
    this.GetApiDataServiceService.getWebApiData_ShiftRoteCheckByWeekType(ShiftRoteCheckByWeekTypeGetApi)
      .pipe(takeWhile(() => this.api_subscribe))
      .subscribe((x: any) => {
        if (x.length == 0) {
          this.showSimulationDetail();
        }
        else {
          alert(x);
          this.LoadingPage.hide();
  ...
  private showSimulationDetail() {
    //模擬調班後->到最後確認頁面
    var GetShiftRoteTwoPersonAfterGetApi: GetShiftRoteTwoPersonAfterGetApiClass[] = []
    for (let data of this.uiShow) {
      if (data.Clickselect) {

        GetShiftRoteTwoPersonAfterGetApi.push({
          ShiftDate: data.reallydate,
          RoteID1: parseInt(data.onePRoteID),
          RoteID2: parseInt(data.twoPRoteID)
        }
        )
      }
    }

    this.GetApiDataServiceService.getWebApiData_GetShiftRoteTwoPersonAfter(GetShiftRoteTwoPersonAfterGetApi)
      .pipe(takeWhile(() => this.api_subscribe))
      .subscribe(
        (x: any) => {
          var canNext = true
          this.uiShow_Simulation = JSON.parse(JSON.stringify(this.uiShow))
          for (let oneSimulation of this.uiShow_Simulation) {
            for (let data of x) {
              if (oneSimulation.reallydate == data.ShiftDate) {
                if (data.RoteCode1 && data.RoteCode2) {
                  oneSimulation.onePRoteID = data.RoteID1
                  oneSimulation.oneP = data.RoteCode1
                  oneSimulation.onePRoteName = data.RoteName1
                  oneSimulation.twoPRoteID = data.RoteID2
                  oneSimulation.twoP = data.RoteCode2
                  oneSimulation.twoPRoteName = data.RoteName2
                  oneSimulation.oneP_OnTime = data.Rote1Info.OnTime
                  oneSimulation.oneP_OffTime = data.Rote1Info.OffTime
                  oneSimulation.twoP_OnTime = data.Rote2Info.OnTime
                  oneSimulation.twoP_OffTime = data.Rote2Info.OffTime
                } else {
                  canNext = false
                  if (!data.RoteCode2) {
  ...
  sendandSaveChangeform() {
    ...
    var ShiftRoteSaveAndFlowStart: NoSameTimeShiftRoteSaveAndFlowStartClass = {
      "FlowApp": {
        "ShiftRoteType": "RR",
        "ShiftRoteName": "雙人互調",
        ...
        "State": "1", //1是要簽核
        "Cond4": "1", //1是不等工時調班(雙人時薪人員與正班人員互調)
  ...
  SimulationRote() {
    ...
    if (!this.SimulationRoteClickOne) {
      ...
      this.ShowDetail = JSON.parse(JSON.stringify(this.uiShow_Simulation));
