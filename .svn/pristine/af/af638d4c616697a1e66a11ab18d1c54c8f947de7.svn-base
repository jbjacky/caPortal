	...
	$.fn.dateDropper = function (options) {
		return $(this).each(function () {
			if ($(this).is('input') && !$(this).hasClass('picker-input')) {
				...
				var values = $.extend({
					minYear: picker_min_year,
					maxYear: picker_max_year,
					today_m: (new Date().getMonth() + 1),
					today_d: new Date().getDate(),
					today_y: new Date().getFullYear()
				}, options)
				//修改過
				pickers[id] = $.extend({
					...
					lang: picker_lang,
					large: picker_large,
					large_class: picker_large_class,
					lock: picker_lock,
					jump: picker_jump,
					bt_large: false, //修改過-新增為切換行事曆大小按鈕
					key: {
						m: {
							min: 1,
							max: 12,
							current: 1,
							today: values.today_m
						},
						d: {
							min: 1,
							max: 31,
							current: 1,
							today: values.today_d
						},
						y: {
							min: values.minYear,
							max: values.maxYear,
							current: values.minYear,
							today: values.today_y
						},
						l: {
							min: 0,
				...
				$('<div>', {
					class: 'datedropper ' + picker_modal + ' ' + picker_theme + ' ' + picker_fx_class + ' ' + pickers[id].large_class,
					id: id,
					html: $('<div>', {
						class: 'picker'
					})
